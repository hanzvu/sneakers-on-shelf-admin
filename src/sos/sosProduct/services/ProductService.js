...
const deleteProductById = async (id) => {
    const response = await axios.delete(`${BASE_API}/admin/v1/products/delete/${id}`)
    return response;
}

const addProduct = async (product) => {
    const response = await axios.get(`${BASE_API}/admin/v1/products/save`, {
        product
    })
    return response;
} 
...
const getTemplateSaveListProduct = async () => {
    return axios.get(`${BASE_API}/admin/v1/products/dowloadTeamplateFile`,{
        method: 'GET',
        responseType: 'blob',
    })
    .then((res) => {
